    ...
    export class SignatureInfoHelpers {
        ...
        public static getActualSignatureInfoFromCallExpression(ast: TypeScript.ICallExpression, caretPosition: number, typeParameterInformation: IPartiallyWrittenTypeArgumentListInformation): ActualSignatureInfo {
            ...
            if (ast.argumentList.args) {
                parameterMinChar = Math.min(parameterMinChar, ast.argumentList.args.start());
                parameterLimChar = Math.max(parameterLimChar, Math.max(ast.argumentList.args.start(), ast.argumentList.args.end() + ast.argumentList.args.trailingTriviaWidth()));
            }
            ...
            else if (ast.argumentList.args && ast.argumentList.args.nonSeparatorCount() > 0) {
                result.currentParameter = 0;
                for (var index = 0; index < ast.argumentList.args.nonSeparatorCount(); index++) {
                    if (caretPosition > ast.argumentList.args.nonSeparatorAt(index).end() + ast.argumentList.args.nonSeparatorAt(index).trailingTriviaWidth()) {
