...
module TypeScript.Services {
    export class LanguageService implements ILanguageService {
        ...
        public getSignatureAtPosition(fileName: string, position: number): SignatureInfo {
            ...
            TypeScript.Debug.assert(callExpression.argumentList.args !== null, "Expected call expression to have arguments, but it did not");

            var argumentsStart = callExpression.expression.end() + callExpression.expression.trailingTriviaWidth();
            var argumentsEnd = callExpression.argumentList.args.end() + callExpression.argumentList.args.trailingTriviaWidth()
        ...
        public getEmitOutput(fileName: string): TypeScript.EmitOutput {
            ...
            try {
                ...
                console.log("Adding " + semanticDiagnostics.length + " diagnostics to the emitResult");
        ...
        private getAllSemanticDiagnostics(): TypeScript.Diagnostic[] {
            ...
            this.compiler.fileNames().map(fileName => {
                ...
                return diagnostics.push.apply(diagnostics, this.compiler.getSemanticDiagnostics(fileName));
            });
