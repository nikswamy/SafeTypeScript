...
module TypeScript {
    export interface ISlidingWindowSource {
        ...
        // the return value.
        fetchMoreItems(argument: any, sourceIndex: number, window: CheckedArray<any>, destinationIndex: number, spaceAvailable: number): number;
    }
    ...
    export class SlidingWindow {
        ...
        constructor(// Underlying source that we retrieve items from.
                    ...
                    public window: CheckedArray<any>,
                    // The default value to return when there are no more items left in the window.
                    private defaultValue: any,
                    //copy
                    private copy : (win: CheckedArray<any>, sourceIndex: number, dest: CheckedArray<any>, destinationIndex: number, length: number) => void,
                    //grow
                    private grow: (win: CheckedArray<any>, length: number, defaultValue:any) => void, 
        ...
        private tryShiftOrGrowWindow(): void {
            ...
            if (currentIndexIsPastWindowHalfwayPoint && isAllowedToShift) {
                ...
                if (shiftCount > 0) {
                    this.copy(this.window, shiftStartIndex, this.window, 0, shiftCount);
                    //NS: used to be ... 
                    //ArrayUtilities.copy(<any>this.window, shiftStartIndex, <any>this.window, 0, shiftCount);
                }
                ...
                this.grow(this.window, this.window.length * 2, this.defaultValue);
                //NS: used to be ... ArrayUtilities.grow<number>(<any>this.window, this.window.length * 2, this.defaultValue);
            }
        }
