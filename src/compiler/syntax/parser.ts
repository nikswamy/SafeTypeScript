...
module TypeScript.Parser {
    ...
    class NormalParserSource implements IParserSource, ISlidingWindowSource {
        ...
        constructor(fileName: string,
            ...
            var copy = (_win: CheckedArray<any>, sourceIndex: number, _dest: CheckedArray<any>, destinationIndex: number, length: number) => {
                var win = RT.forceCheckedArray<any, ISyntaxToken>(_win);
                var dest = RT.forceCheckedArray<any, ISyntaxToken>(_dest);
                ArrayUtilities.copy<ISyntaxToken>(win, sourceIndex, dest, destinationIndex, length);
            };
            var grow = (_win: CheckedArray<any>, length: number, def:any) => {
                var win = RT.forceCheckedArray<any, ISyntaxToken>(_win);
                ArrayUtilities.grow<ISyntaxToken>(win, length, <ISyntaxToken>def);
            };      
            this.slidingWindow = new SlidingWindow(this, ArrayUtilities.createArray<ISyntaxToken>(/*defaultWindowSize:*/ 32, null), null, copy, grow);
        ...
        public fetchMoreItems(allowRegularExpression: any, sourceIndex: number, _window: CheckedArray<any>, destinationIndex: number, spaceAvailable: number): number {
            // Assert disabled because it is actually expensive enugh to affect perf.
            // Debug.assert(spaceAvailable > 0);
            var window: ISyntaxToken[] = RT.forceCheckedArray<any,ISyntaxToken>(_window);
    ...
    class ParserImpl {
        ...
        private parseSourceUnit(): SourceUnitSyntax {
            ...
            var result = this.parseSyntaxList(ListParsingState.SourceUnit_ModuleElements, (parser, items) => ParserImpl.updateStrictModeState(parser, items));
        ...
        private static updateStrictModeState(parser: ParserImpl, items: CheckedArray<any>): void {
        ...
        private parseModifiers(): ISyntaxList {
            var tokens: ISyntaxToken[] = this.getSyntaxTokenArray();
            ...
            this.returnZeroOrOneLengthSyntaxTokenArray(tokens);
        ...
        private parseMemberFunctionDeclaration(): MemberFunctionDeclarationSyntax {
            ...
            var modifierArray: ISyntaxToken[] = this.getSyntaxTokenArray();
        ...
        private parseBlock(parseBlockEvenWithNoOpenBrace: boolean, checkForStrictMode: boolean): BlockSyntax {
            ...
            if (parseBlockEvenWithNoOpenBrace || openBraceToken.width() > 0) {
                ...
                var processItems = checkForStrictMode ? (parser:ParserImpl, items:CheckedArray<any>) => ParserImpl.updateStrictModeState(parser, items) : <(p:ParserImpl, i:CheckedArray<any>) => void>null;
        ...
        private parseSyntaxList(currentListType: ListParsingState,
                                processItems: (parser: ParserImpl, items: CheckedArray<any>) => void = null): { skippedTokens: ISyntaxToken[]; list: ISyntaxList; } {
            var savedListParsingState = this.listParsingState;
        ...
        private tryParseExpectedListItem(currentListType: ListParsingState,
                                         ...
                                         items: ISyntaxNodeOrToken[],
                                         processItems: (parser: ParserImpl, items: CheckedArray<any>) => void ): void {
        ...
        private arraySyntaxNodePool: SyntaxNode[][] = [];
        private getSyntaxNodeArray(): SyntaxNode[] {
            if (this.arraySyntaxNodePool.length > 0) {
                return this.arraySyntaxNodePool.pop();
            }

            return [];
        }

        private returnZeroOrOneLengthSyntaxNodeArray(array: SyntaxNode[]) {
            if (array.length <= 1) {
                this.returnSyntaxNodeArray(array);
            }
        }

        private returnSyntaxNodeArray(array: SyntaxNode[]) {
            array.length = 0;
            this.arraySyntaxNodePool.push(array);
        }

        /////////////////////////////////////////////////////////////////////////////////////////////////


        private arraySyntaxTokenPool: ISyntaxToken[][] = [];
        private getSyntaxTokenArray(): ISyntaxToken[] {
            if (this.arraySyntaxTokenPool.length > 0) {
                return this.arraySyntaxTokenPool.pop();
            }

            return [];
        }

        private returnZeroOrOneLengthSyntaxTokenArray(array: ISyntaxToken[]) {
            if (array.length <= 1) {
                this.returnSyntaxTokenArray(array);
            }
        }

        private returnSyntaxTokenArray(array: ISyntaxToken[]) {
            array.length = 0;
            this.arraySyntaxTokenPool.push(array);
        }

        /////////////////////////////////////////////////////////////////////////////////////////////////


        private arraySyntaxNodeOrTokenPool: ISyntaxNodeOrToken[][] = [];
        private getSyntaxNodeOrTokenArray(): ISyntaxNodeOrToken[] {
            if (this.arraySyntaxNodeOrTokenPool.length > 0) {
                return this.arraySyntaxNodeOrTokenPool.pop();
            }

            return [];
        }

        private returnZeroOrOneLengthSyntaxNodeOrTokenArray(array: ISyntaxNodeOrToken[]) {
            if (array.length <= 1) {
                this.returnSyntaxNodeOrTokenArray(array);
            }
        }

        private returnSyntaxNodeOrTokenArray(array: ISyntaxNodeOrToken[]) {
            array.length = 0;
            this.arraySyntaxNodeOrTokenPool.push(array);
        }
        ...
        private parseSyntaxListWorker(currentListType: ListParsingState,
                                      processItems: (parser: ParserImpl, items: CheckedArray<any>) => void ): { skippedTokens: ISyntaxToken[]; list: ISyntaxList; } {
            var items = this.getSyntaxNodeOrTokenArray();
            var skippedTokens: ISyntaxToken[] = this.getSyntaxTokenArray();
            ...
            this.returnZeroOrOneLengthSyntaxNodeOrTokenArray(items);
        ...
        private parseSeparatedSyntaxListWorker(currentListType: ListParsingState): { skippedTokens: ISyntaxToken[]; list: ISeparatedSyntaxList; } {
            var items: ISyntaxNodeOrToken[] = this.getSyntaxNodeOrTokenArray();
            var skippedTokens: ISyntaxToken[] = this.getSyntaxTokenArray();
