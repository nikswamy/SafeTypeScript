    ...
    export class Scanner implements ISlidingWindowSource {
        ...
        constructor(fileName: string,
            ...
            var copy = (_win: CheckedArray<any>, sourceIndex: number, _dest: CheckedArray<any>, destinationIndex: number, length: number) => {
                var win = RT.forceCheckedArray<any, number>(_win);
                var dest = RT.forceCheckedArray<any, number>(_dest);
                ArrayUtilities.copy<number>(win, sourceIndex, dest, destinationIndex, length);
            }; 
            var grow = (_win: CheckedArray<any>, length: number, def:any) => {
                var win = RT.forceCheckedArray<any, number>(_win);
                ArrayUtilities.grow<number>(win, length, +def);
            };      
            this.slidingWindow = new SlidingWindow(this, window, 0, copy, grow, text.length());
        ...
        private tryFastScanIdentifierOrKeyword(firstCharacter: number): SyntaxKind {
            ...
            var window: number[] = RT.forceCheckedArray<any,number>(slidingWindow.window);
        ...
        public substring(start: number, end: number, intern: boolean): string {
            ...
            var window: number[] = RT.forceCheckedArray<any, number>(this.slidingWindow.window);
            if (intern) {
                return Collections.DefaultStringTable.addCharArray(window, offset, length);
            }
            else {
                return StringUtilities.fromCharCodeArray(window.slice(offset, offset + length));
            }
        }
