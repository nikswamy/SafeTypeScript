...
module TypeScript {
    ...
    //declare var IO: any;                                NS: redundant declaration (see io.ts, line 120)
    ...
    export var soundTypeCheckTime = 0;
    ...
    export class TypeScriptCompiler {
        ...
        private emitDocumentDeclarationsWorker(
            ...
            if (declarationEmitter) {
                ...
                var declareFileName = this.mapOutputFileName(document, emitOptions, (fn, wnr) => TypeScriptCompiler.mapToDTSFileName(fn, wnr));
        ...
        private emitDocumentWorker(document: Document, emitOptions: EmitOptions, emitter?: Emitter): Emitter {
            ...
            if (!emitter) {
                var javaScriptFileName = this.mapOutputFileName(document, emitOptions, (fn, wnr) => TypeScriptCompiler.mapToJSFileName(fn, wnr));
        ...
        public emitAll(resolvePath: (path: string) => string): EmitOutput {
            ...
            for (var i = 0, n = fileNames.length; i < n; i++) {
                ...
                sharedEmitter = this._emitDocument(document, emitOptions,
                    files => RT.applyVariadic<OutputFile>(emitOutput.outputFiles, "push", files),
                            //emitOutput.outputFiles.push.apply(emitOutput.outputFiles, files),
                    sharedEmitter);
            }

            if (sharedEmitter) {
                RT.applyVariadic<OutputFile>(emitOutput.outputFiles, "push", sharedEmitter.getOutputFiles());
        ...
        public emit(fileName: string, resolvePath: (path: string) => string): EmitOutput {
            ...
            if (document.emitToOwnOutputFile()) {
                this._emitDocument(document, emitOptions,
                    files => RT.applyVariadic<OutputFile>(emitOutput.outputFiles, "push", files), /*sharedEmitter:*/ null);
        ...
        private processDiagnostics(errors: Diagnostic[]) {
            errors = ArrayUtilities.distinct(errors, (d1, d2) => Diagnostic.equals(d1, d2));
            errors.sort((d1, d2) => {
        ...
        public resolveAllFiles() {
            ...
            if (this.compilationSettings().safe() || this.compilationSettings().secure()) {
                if (this.compilationSettings().serviceMode()) {
                    ...
                    allFiles.forEach((fn) => {
                        ...
                        if (sn === "lib.d.ts") {
                            ...
                            PullTypeResolver.typeCheck(this.compilationSettings(), this.semanticInfoChain, libDoc);
                ...
                fileNames.forEach((fn) => { this.buildSignature(fn); }); //first, process all type declarations to build a signature
                    ...
                    case SyntaxKind.ObjectCreationExpression:
                        if (propagateContextualTypes) {
                            ...
                            if ((i + 2 < n) && callExpression.argumentList === path[i + 1] && callExpression.argumentList.args === path[i + 2]) {
                                ...
                                if (isNew) {
                                    resolver.resolveObjectCreationExpression(<ObjectCreationExpression>callExpression, resolutionContext, callResolutionResults);
                                }
                                else {
                                    resolver.resolveInvocationExpression(<InvocationExpression>callExpression, resolutionContext, callResolutionResults);
                                }
                                ...
                                if (isNew) {
                                    resolver.resolveObjectCreationExpression(<ObjectCreationExpression>(<any>callExpression), resolutionContext);
                                }
                                else {
                                    resolver.resolveInvocationExpression(<InvocationExpression>(<any>callExpression), resolutionContext);
                                }
                            }
