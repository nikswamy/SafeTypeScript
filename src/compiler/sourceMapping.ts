...
module TypeScript {
    ...
    export class SourceMapper {
        ...
        public decreaseMappingLevel(ast: IASTSpan) {
            ...
            var expectedAstInfo: any = (expectedAst instanceof AST) ? SyntaxKind[(<AST>expectedAst).kind()] : [expectedAst.start(), expectedAst.end()];
            var astInfo: any = (ast instanceof AST) ? SyntaxKind[(<AST>ast).kind()] : [ast.start(), ast.end()]
            Debug.assert(
                ast === expectedAst,
        ...
        public emitSourceMapping(): void {
            Debug.assert(
                ...
                this.mappingLevel.map(item => [<any>'Node of type', SyntaxKind[(<AST>item).kind()], 'at', item.start(), 'to', item.end()].join(' ')).join(', '));
