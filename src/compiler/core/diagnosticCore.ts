...
module TypeScript {
    ...
    export class Diagnostic {
        ...
        constructor(fileName: string, lineMap: LineMap, start: number, length: number, diagnosticKey: string, args: string[]= null) {
    ...
    export function getLocalizedText(diagnosticKey: string, args: string[]): string {
        ...
        if (expectedCount !== actualCount) {
            throw new Error(diagnosticKey + " :: " + getLocalizedText(DiagnosticCode.Expected_0_arguments_to_message_got_1_instead, [expectedCount.toString(), actualCount.toString()]));
        }
        ...
        if (valueCount !== expectedCount) {
            throw new Error(diagnosticKey + " :: " + getLocalizedText(DiagnosticCode.Expected_the_message_0_to_have_1_arguments_but_it_had_2, [diagnosticMessageText, expectedCount.toString(), valueCount.toString()]));
        }
    ...
    export function getDiagnosticMessage(diagnosticKey: string, args: string[]): string {
        ...
        if (diagnostic.category === DiagnosticCategory.Error) {
            message = getLocalizedText(DiagnosticCode.error_TS_0_1, [diagnostic.code.toString(), diagnosticMessageText]);
        } else if (diagnostic.category === DiagnosticCategory.Warning) {
            message = getLocalizedText(DiagnosticCode.warning_TS_0_1, [diagnostic.code.toString(), diagnosticMessageText]);
        } else {
