...
module TypeScript {
    ...
    function computeEnumElementConstantValues(ast: EnumDeclaration, enumDecl: PullDecl, context: DeclCollectionContext): void {
        ...
        for (var i = 0, n = ast.enumElements.nonSeparatorCount(); i < n; i++) {
            ...
            var enumElementDecl = <PullEnumElementDecl>ArrayUtilities.first(enumMemberDecls, d =>
    ...
    function computeEnumElementConstantValue(expression: AST, enumMemberDecls: PullDecl[], context: DeclCollectionContext): number {
        ...
        if (isIntegerLiteralAST(expression)) {
            ...
            switch (expression.kind()) {
                case SyntaxKind.IdentifierName:
                    ...
                    var matchingEnumElement = <PullEnumElementDecl>ArrayUtilities.firstOrDefault(enumMemberDecls, d => d.name === name.valueText());
