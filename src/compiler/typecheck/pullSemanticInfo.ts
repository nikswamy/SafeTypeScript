...
module TypeScript {
    ...
    export class SemanticInfoChain {
        ...
        public findDecls(declPath: string[], declKind: PullElementKind): PullDecl[] {
            ...
            for (var i = 0; i < declPath.length; i++) {
                ...
                for (var j = 0; j < declsToSearch.length; j++) {
                    //var kind = (i === declPath.length - 1) ? declKind : PullElementKind.SomeType; NS: Why was this commented out?
                    foundDecls = declsToSearch[j].searchChildDecls(path, i===(declPath.length - 1) ? declKind : PullElementKind.Container);
        ...
        public addDiagnosticFromAST(ast: AST, diagnosticKey: string, args: string[]= null): void {
            this.addDiagnostic(this.diagnosticFromAST(ast, diagnosticKey, args));
        }

        public diagnosticFromAST(ast: AST, diagnosticKey: string, args: string[]= null): Diagnostic {
            return new Diagnostic(ast.fileName(), this.lineMap(ast.fileName()), ast.start(), ast.width(), diagnosticKey, args);
        }
    }
}
