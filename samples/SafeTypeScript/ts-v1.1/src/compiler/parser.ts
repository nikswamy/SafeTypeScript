...
module ts {
    ...
    export function forEachChild<T>(node: Node, cbNode: (node: Node) => T): T {//, cbNodes?: (nodes: CheckedArray<Node> /*AR: Node[]*/) => T): T {
        function child(node: Node): T {
            if (node) return cbNode(node);
        }
        function children<U>(ns: NodeArray<U>) {//CheckedArray<Node> /*AR: Node[]*/) {
            var nodes = __trustedcast__<NodeArray<U>, Node[]>(ns);
            if (nodes) {
        ...
        switch (node.kind) {
            case SyntaxKind.QualifiedName:
                var qnode = <QualifiedName> node;
                return child(qnode.left) ||
                    child(qnode.right);
            case SyntaxKind.TypeParameter:
                var tnode = <TypeParameterDeclaration> node;
                return child(tnode.name) ||
                    child(tnode.constraint);
            case SyntaxKind.Parameter:
                var pnode = <ParameterDeclaration> node;
                return child(pnode.name) ||
                    child(pnode.type) ||
                    child(pnode.initializer);
            case SyntaxKind.Property:
            case SyntaxKind.PropertyAssignment:
                var pnode = <PropertyDeclaration> node;
                return child(pnode.name) ||
                    child(pnode.type) ||
                    child(pnode.initializer);
            ...
            case SyntaxKind.IndexSignature:
                var snode = <SignatureDeclaration> node;
                return children(snode.typeParameters) ||
                    children(snode.parameters) ||
                    child(snode.type);
            ...
            case SyntaxKind.ArrowFunction:
                var fnode = <FunctionDeclaration> node;
                return child(fnode.name) ||
                    children(fnode.typeParameters) ||
                    children(fnode.parameters) ||
                    child(fnode.type) ||
                    child(fnode.body);
            case SyntaxKind.TypeReference:
                var tynode = <TypeReferenceNode> node;
                return child(tynode.typeName) ||
                    children(tynode.typeArguments);
            ...
            case SyntaxKind.PropertyAccess:
                var panode = <PropertyAccess> node;
                return child(panode.left) ||
                    child(panode.right);
            case SyntaxKind.IndexedAccess:
                var inode = <IndexedAccess> node;
                return child(inode.object) ||
                    child(inode.index);
            case SyntaxKind.CallExpression:
            case SyntaxKind.NewExpression:
                var cnode = <CallExpression> node;
                return child(cnode.func) ||
                    children(cnode.typeArguments) ||
                    children(cnode.arguments);
            case SyntaxKind.TypeAssertion:
                var tanode = <TypeAssertion> node;
                return child(tanode.type) ||
                    child(tanode.operand);
            ...
            case SyntaxKind.BinaryExpression:
                ...
                return child(bnode.left) ||
                    child(bnode.right);
            case SyntaxKind.ConditionalExpression:
                var conode = <ConditionalExpression> node;
                return child(conode.condition) ||
                    child(conode.whenTrue) ||
                    child(conode.whenFalse);
            ...
            case SyntaxKind.IfStatement:
                ...
                return child(ifnode.expression) ||
                    child(ifnode.thenStatement) ||
                    child(ifnode.elseStatement);
            case SyntaxKind.DoStatement:
                var donode = <DoStatement> node;
                return child(donode.statement) ||
                    child(donode.expression);
            case SyntaxKind.WhileStatement:
                var wnode = <WhileStatement> node;
                return child(wnode.expression) ||
                    child(wnode.statement);
            case SyntaxKind.ForStatement:
                var fonode = <ForStatement> node;
                return children(fonode.declarations) ||
                    child(fonode.initializer) ||
                    child(fonode.condition) ||
                    child(fonode.iterator) ||
                    child(fonode.statement);
            case SyntaxKind.ForInStatement:
                var finode = <ForInStatement> node;
                return child(finode.declaration) ||
                    child(finode.variable) ||
                    child(finode.expression) ||
                    child(finode.statement);
            ...
            case SyntaxKind.WithStatement:
                var winode = <WithStatement> node;
                return child(winode.expression) ||
                    child(winode.statement);
            case SyntaxKind.SwitchStatement:
                var swnode = <SwitchStatement> node;
                return child(swnode.expression) ||
                    children(swnode.clauses);
            case SyntaxKind.CaseClause:
            case SyntaxKind.DefaultClause:
                var codnode = <CaseOrDefaultClause> node;
                return child(codnode.expression) ||
                    children(codnode.statements);
            case SyntaxKind.LabelledStatement:
                var lnode = <LabelledStatement> node;
                return child(lnode.label) ||
                    child(lnode.statement);
            case SyntaxKind.ThrowStatement:
                return child((<ThrowStatement>node).expression);
            case SyntaxKind.TryStatement:
                var trnode = <TryStatement> node;
                return child(trnode.tryBlock) ||
                    child(trnode.catchBlock) ||
                    child(trnode.finallyBlock);
            case SyntaxKind.CatchBlock:
                var canode = <CatchBlock> node;
                return child(canode.variable) ||
                    children(canode.statements);
            case SyntaxKind.VariableDeclaration:
                var vdnode = <VariableDeclaration> node;
                return child(vdnode.name) ||
                    child(vdnode.type) ||
                    child(vdnode.initializer);
            case SyntaxKind.ClassDeclaration:
                var cdnode = <ClassDeclaration> node;
                return child(cdnode.name) ||
                    children(cdnode.typeParameters) ||
                    child(cdnode.baseType) ||
                    children(cdnode.implementedTypes) ||
                    children(cdnode.members);
            case SyntaxKind.InterfaceDeclaration:
                var idnode = <InterfaceDeclaration> node;
                return child(idnode.name) ||
                    children(idnode.typeParameters) ||
                    children(idnode.baseTypes) ||
                    children(idnode.members);
            case SyntaxKind.EnumDeclaration:
                var ennode = <EnumDeclaration> node;
                return child(ennode.name) ||
                    children(ennode.members);
            case SyntaxKind.EnumMember:
                var emnode = <EnumMember> node;
                return child(emnode.name) ||
                    child(emnode.initializer);
            case SyntaxKind.ModuleDeclaration:
                var monode = <ModuleDeclaration> node;
                return child(monode.name) ||
                    child(monode.body);
            case SyntaxKind.ImportDeclaration:
                var imnode = <ImportDeclaration> node;
                return child(imnode.name) ||
                    child(imnode.entityName) ||
                    child(imnode.externalModuleName);
    ...
    export function createSourceFile(filename: string, sourceText: string, languageVersion: ScriptTarget): SourceFile {
        ...
        function createIdentifier(isIdentifier: boolean): Identifier {
            ...
            if (isIdentifier) {
                ...
                node.text = hasProperty(identifiers, text) ? identifiers[text] : (identifiers[text] = text); //AR: without noLib option, this gives an error, tries to add hasOwnProperty in the identifiers!
                nextToken();
        ...
        function parseList<T extends Node>(kind: ParsingContext, checkForStrictMode: boolean, parseElement: () => T): NodeArray<T> {
            ...
            var result = __trustedcast__<Array<T>, NodeArray<T>>([]);
                ...
                if (isListElement(kind, /* inErrorRecovery */ false)) {
                    ...
                    if (commaStart >= 0) {
                        if (trailingCommaBehavior === TrailingCommaBehavior.Disallow) {
                            ...
                            result.push(__trustedcast__<Node, T>(createNode(SyntaxKind.OmittedExpression)));
                        }
                    }
        ...
        function createNodeArray<T extends Node>(node: T): NodeArray<T> {
            var result = __trustedcast__<Array<T>, NodeArray<T>>([node]);
            result.pos = node.pos;
            result.end = node.end;
            return result;
        }
        ...
        function parsePropertyOrMethod(): Declaration {
            ...
            if (token === SyntaxKind.OpenParenToken || token === SyntaxKind.LessThanToken) {
                ...
                md.name = name;
                md.typeParameters = sig.typeParameters;
                md.parameters = sig.parameters;
                md.type = sig.type;
            }
            else {
                node.kind = SyntaxKind.Property;
                var pd = <PropertyDeclaration> node;
                pd.type = parseTypeAnnotation();
                pd.name = name;
            }
            parseSemicolon();
            return finishNode(node);
        ...
        function parseTypeLiteral(): TypeLiteralNode {
            ...
            if (parseExpected(SyntaxKind.OpenBraceToken)) {
                node.members = __trustedcast__<NodeArray<Declaration>, NodeArray<Node>>(parseList(ParsingContext.TypeMembers, /*checkForStrictMode*/ false, parseTypeMember));
        ...
        function parseFunctionType(signatureKind: SyntaxKind): TypeLiteralNode {
            ...
            finishNode(member);
            node.members = __trustedcast__<NodeArray<SignatureDeclaration>, NodeArray<Node>>(createNodeArray(member));
            return finishNode(node);
        }
        ...
        function parseObjectLiteral(): ObjectLiteral {
            ...
            forEach(node.properties, (q: Node) => {
                var p: Declaration = <Declaration> q;
        ...
        function parseClassDeclaration(pos: number, flags: NodeFlags): ClassDeclaration {
            ...
            if (parseExpected(SyntaxKind.OpenBraceToken)) {
                node.members = __trustedcast__<NodeArray<Declaration>, NodeArray<Node>>(parseList(ParsingContext.ClassMembers, /*checkForStrictMode*/ false, parseClassMemberDeclaration));
                parseExpected(SyntaxKind.CloseBraceToken);
            }
            else {
