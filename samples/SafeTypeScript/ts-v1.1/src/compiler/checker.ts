...
module ts {
    ...
    export function createTypeChecker(program: Program): TypeChecker {
        ...
        var emptyObjectType = createAnonymousType(undefined, emptySymbols, __trustedcast__<any[], Signature[]>(emptyArray), __trustedcast__<any[], Signature[]>(emptyArray), undefined, undefined);
        var anyFunctionType = createAnonymousType(undefined, emptySymbols, __trustedcast__<any[], Signature[]>(emptyArray), __trustedcast__<any[], Signature[]>(emptyArray), undefined, undefined);
        var noConstraintType = createAnonymousType(undefined, emptySymbols, __trustedcast__<any[], Signature[]>(emptyArray), __trustedcast__<any[], Signature[]>(emptyArray), undefined, undefined);
        ...
        function createSymbol(flags: SymbolFlags, name: string): Symbol {
            return tagSymbol(new Symbol(flags, name));
        }
        ...
        function resolveEntityName(location: Node, name: EntityName, meaning: SymbolFlags): Symbol {
            ...
            if (name.kind === SyntaxKind.Identifier) {
                ...
                symbol = resolveName(location, (<Identifier>name).text, meaning, Diagnostics.Cannot_find_name_0, identifierToString(<Identifier>name));
                if (!symbol) {
                    return;
                }
            }
                ...
                symbol = getSymbol(namespace.exports, (<QualifiedName>name).right.text, meaning);
                if (!symbol) {
            ...
            if (!containerSymbol.exportAssignSymbol) {
                ...
                if (exportInformation.exportAssignments.length) {
                    ...
                    if (node.exportName.text) {
                        ...
                        exportSymbol = resolveName(node, node.exportName.text, meaning, Diagnostics.Cannot_find_name_0, identifierToString(node.exportName));
                    }
                }
        ...
        function collectExportInformationForSourceFileOrModule(symbol: Symbol) {
            ...
            forEach(symbol.declarations, declaration => {
                var block = declaration.kind === SyntaxKind.SourceFile ? <Block> declaration : <Block> ((<ModuleDeclaration>declaration).body); //AR: if Block cast outside, TS infers decl to be the type ?
        ...
        function getNamedMembers(members: SymbolTable): Symbol[] {
            ...
            return result || __trustedcast__<any[], Symbol[]>(emptyArray);
        }
        ...
        function setObjectTypeMembers(type: ObjectType, members: SymbolTable, callSignatures: Signature[], constructSignatures: Signature[], stringIndexType: Type, numberIndexType: Type): ResolvedObjectType {
            var rt = <ResolvedObjectType> type;
            rt.members = members;
            rt.properties = getNamedMembers(members);
            rt.callSignatures = callSignatures;
            rt.constructSignatures = constructSignatures;
            if (stringIndexType) rt.stringIndexType = stringIndexType;
            if (numberIndexType) rt.numberIndexType = numberIndexType;
            return rt;
        }
        ...
        function hasBaseType(type: InterfaceType, checkBase: InterfaceType) {
            return check(type);
            function check(t: ObjectType): boolean { //AR
                var type = <InterfaceType> t;
        ...
        function getDeclaredTypeOfClass(symbol: Symbol): InterfaceType {
            ...
            if (!links.declaredType) {
                ...
                if (typeParameters) {
                    ...
                    var gt = <GenericType> type;
                    gt.instantiations = {};
                    gt.instantiations[getTypeListId(type.typeParameters)] = <GenericType>type;
                    gt.target = <GenericType>type;
                    gt.typeArguments = type.typeParameters;
                }
                type.baseTypes = [];
                ...
                type.declaredCallSignatures = __trustedcast__<any[], Signature[]>(emptyArray);
                type.declaredConstructSignatures = __trustedcast__<any[], Signature[]>(emptyArray);
        ...
        function getDeclaredTypeOfInterface(symbol: Symbol): InterfaceType {
            ...
            if (!links.declaredType) {
                ...
                if (typeParameters) {
                    ...
                    var gt = <GenericType> type;
                    gt.instantiations = {};
                    gt.instantiations[getTypeListId(type.typeParameters)] = gt;
                    gt.target = gt;
                    gt.typeArguments = type.typeParameters.map((t) => <Type> t);
                }
                type.baseTypes = [];
                forEach(symbol.declarations, declaration => {
        ...
        function getDefaultConstructSignatures(classType: InterfaceType): Signature[] {
            ...
            return [createSignature(undefined, classType.typeParameters, __trustedcast__<any[], Symbol[]>(emptyArray), classType, 0, false, false)];
        }

        function resolveAnonymousTypeMembers(type: ObjectType) {
            var symbol = type.symbol;
            var members = emptySymbols;
            var callSignatures: Signature[] = __trustedcast__<any[], Signature[]>(emptyArray);
            var constructSignatures: Signature[] = __trustedcast__<any[], Signature[]>(emptyArray);
        ...
        function getPropertiesOfType(type: Type): Symbol[] {
            ...
            return __trustedcast__<any[], Symbol[]>(emptyArray);
        }
        ...
        function getSignaturesOfType(type: Type, kind: SignatureKind): Signature[] {
            ...
            return __trustedcast__<any[], Signature[]>(emptyArray);
        }
        ...
        function getSignaturesOfSymbol(symbol: Symbol): Signature[] {
            if (!symbol) return __trustedcast__<any[], Signature[]>(emptyArray);
        ...
        function getReturnTypeOfSignature(signature: Signature): Type {
            if (!signature.resolvedReturnType) {
                ...
                if (signature.target) {
                    ...
                    type = getReturnTypeFromBody(<FunctionDeclaration>signature.declaration);
                }
        ...
        function getOrCreateTypeFromSignature(signature: Signature): ObjectType {
            ...
            if (!signature.isolatedSignatureType) {
                ...
                type.properties = __trustedcast__<any[], Symbol[]>(emptyArray);
                type.callSignatures = !isConstructor ? [signature] : __trustedcast__<any[], Signature[]>(emptyArray);
                type.constructSignatures = isConstructor ? [signature] : __trustedcast__<any[], Signature[]>(emptyArray);
        ...
        function getTypeListId(types: CheckedArray<Type>/*Type[]*/) {
            switch (types.length) {
                case 1:
                    return "" + types[0].id;
                case 2:
        ...
        function createTypeMapper(sources: CheckedArray<Type>/*Type[]*/, targets: Type[]): TypeMapper {
        ...
        function getTypeWithoutConstructors(type: Type): Type {
            if (type.flags & TypeFlags.ObjectType) {
                ...
                if (resolved.constructSignatures.length) {
                    ...
                    result.constructSignatures = __trustedcast__<any[], Signature[]>(emptyArray);
                    type = result;
                }
            }
            return type;
        ...
        function checkInheritedPropertiesAreIdentical(type: InterfaceType, typeNode: Node): boolean {
            ...
            forEach(type.declaredProperties, p => { seen[p.name] = { prop: p, containingType: <Type> type }; });
            var ok = true;
            ...
            for (var i = 0, len = type.baseTypes.length; i < len; ++i) {
                ...
                for (var j = 0, proplen = properties.length; j < proplen; ++j) {
                    ...
                    if (!hasProperty(seen, prop.name)) {
                        seen[prop.name] = { prop: prop, containingType: <Type> base };
                    }
                    else {
                ...
                if (relation === identityRelation) {
                    ...
                    if (targetType) {
                        ...
                        if (sourceStringType && sourceNumberType) {
                            ...
                            compatible = isRelatedTo(sourceStringType, targetType, false) || isRelatedTo(sourceNumberType, targetType, reportErrors);
                        }
                        else {
        ...
        function getWidenedTypeOfObjectLiteral(type: Type): Type {
            ...
            if (properties.length) {
                ...
                if (propTypeWasWidened) {
                    ...
                    type = createAnonymousType(type.symbol, members, __trustedcast__<any[], Signature[]>(emptyArray), __trustedcast__<any[], Signature[]>(emptyArray), stringIndexType, numberIndexType);
                }
            }
            return type;
        }
        ...
        function checkSuperExpression(node: Node, isCallExpression: boolean): Type {
            ...
            var needToCaptureLexicalThis: boolean;
            if (container) {
        ...
        function isNumericName(name: string) {
            ...
            return !isNaN(+name); //AR: isNaN should be any to boolean ?
        }
        ...
        function checkObjectLiteral(node: ObjectLiteral, contextualType?: Type, contextualMapper?: TypeMapper): Type {
            ...
            return createAnonymousType(node.symbol, properties, __trustedcast__<any[], Signature[]>(emptyArray), __trustedcast__<any[], Signature[]>(emptyArray), stringIndexType, numberIndexType);
        ...
        function isCandidateSignature(node: CallExpression, signature: Signature) {
            var args = node.arguments || __trustedcast__<any[], NodeArray<Expression>>(emptyArray);
        ...
        function checkCall(node: CallExpression, signatures: Signature[]): Type {
            ...
            for (var i = 0; i < args.length; i++) {
                if (isContextSensitiveExpression(args[i])) {
                    if (!excludeArgument) excludeArgument = new Array<boolean>(args.length);
        ...
        function checkConstructorDeclaration(node: ConstructorDeclaration) {
            ...
            if ((<ClassDeclaration>node.parent).baseType) {
                ...
                if (containsSuperCall(node.body)) {
                    ...
                    var superCallShouldBeFirst: any = //AR ?
        ...
        function getNodeAtPosition(sourceFile: SourceFile, position: number): Node {
            ...
            return (<any> findChildAtPosition(sourceFile));
        }
