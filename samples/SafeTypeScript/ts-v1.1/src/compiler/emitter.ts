    ...
    export function emitFiles(resolver: EmitResolver): EmitResult {
        ...
        function getOwnEmitOutputFilePath(sourceFile: SourceFile, extension: string) {
            ...
            if (program.getCompilerOptions().outDir) {
                emitOutputFilePathWithoutExtension = getModuleNameFromFilename(getSourceFilePathInNewDir(program.getCompilerOptions().outDir, sourceFile));
            }
            else {
                emitOutputFilePathWithoutExtension = getModuleNameFromFilename(sourceFile.filename);
            }
        ...
        function emitJavaScript(jsFilePath: string, root?: SourceFile) {
            ...
            function emitCommaList(nodes: CheckedArray<Node>/*Node[]*/, count?: number) {
            ...
            function emitDefaultValueAssignments(node: FunctionDeclaration) {
                forEach(__trustedcast__<NodeArray<ParameterDeclaration>, ParameterDeclaration[]>(node.parameters), param => {
            ...
            function emitSignatureParameters(node: FunctionDeclaration) {
                write("(");
                if (node) {
                    emitCommaList(__trustedcast__<NodeArray<ParameterDeclaration>, ParameterDeclaration[]>(node.parameters), node.parameters.length - (hasRestParameters(node) ? 1 : 0));
                }
                write(")");
            }
