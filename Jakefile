...
/* Compiles a file from a list of sources
	...
	*/
function compileFile(outFile, sources, prereqs, prefixes, suffixes, useBuiltCompiler) {
	file(outFile, prereqs, function() {
		var dir = useBuiltCompiler ? builtLocalDirectory : LKGDirectory;
		var cmd = (process.env.host || process.env.TYPESCRIPT_HOST || "node") + " " + dir + "tsc.js -removeComments -propagateEnumConstants -t ES5 -noImplicitAny --module commonjs " + sources.join(" ") + " -out " + outFile;
		if (useDebugMode) {
...
function copyFile(srcFile, tgtFile, suffixes) {
	file(tgtFile, suffixes.push(srcFile), function() {
...
compileFile(serviceFile, compilerSources.concat(servicesSources), [builtLocalDirectory, thirdParty, copyright].concat(compilerSources).concat(servicesSources), [thirdParty, copyright], [/*servicesTail*/]);
